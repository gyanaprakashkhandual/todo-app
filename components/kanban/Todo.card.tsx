...
export default function TodoCard({ todo, onEdit, onDelete, onPress }: TodoCardProps) {
  ...
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={() => onPress(todo)} style={s.card}>
      {/* Priority accent bar */}
      ...
      <View style={s.content}>
        ...
        <View style={s.topRow}>
          ...
          <View style={[s.badge, { backgroundColor: colors.isDark ? pc.bgColorDark : pc.bgColor, marginLeft: 6 }]}>
          ...
          >
            <Menu.Item
              onPress={() => { setMenuVisible(false); onPress(todo); }}
              title="View Details"
              leadingIcon="eye-outline"
              titleStyle={{ color: colors.text }}
            />
            <Menu.Item
              onPress={() => { setMenuVisible(false); onEdit(todo); }}
              title="Edit"
        ...
        {todo.tags?.length > 0 && (
          <View style={s.tagsRow}>
            ...
            {todo.tags.slice(0, 3).map((tag) => (
              <View key={tag} style={[s.tag, { backgroundColor: colors.surfaceElevated, borderColor: colors.border }]}>
                <Text style={[s.tagText, { color: colors.textSecondary }]}>{tag}</Text>
              </View>
            ))}
            {todo.tags.length > 3 && (
              <Text style={[s.moreTag, { color: colors.textMuted }]}>+{todo.tags.length - 3}</Text>
            )}
          </View>
        )}
        ...
        {/* Due date */}
        {dueDate && (
          <View style={[s.dateRow, overdue && { backgroundColor: colors.dangerBg, borderTopColor: colors.dangerBorder }]}>
            <MaterialCommunityIcons
              name={overdue ? 'alert-circle-outline' : 'calendar-outline'}
              size={12}
              color={overdue ? colors.danger : colors.textMuted}
            />
            <Text style={[s.dateText, { color: overdue ? colors.danger : colors.textSecondary }]}>
              {overdue ? 'Overdue · ' : 'Due '}
              {dueDate}
            </Text>
          </View>
...
const styles = (colors: ReturnType<typeof import('../hooks/useColors').useColors>) =>
  StyleSheet.create({
    ...
    content: { paddingLeft: 16, paddingRight: 12, paddingTop: 12, paddingBottom: 0 },
    ...
    tagsRow: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 8, flexWrap: 'wrap' },
    tag: {
      paddingHorizontal: 6,
      paddingVertical: 2,
      borderRadius: 4,
