...
export const fetchStats = createAsyncThunk<TodoStats, void, { rejectValue: string }>(
  'todos/fetchStats',
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue((e as Error).message ?? 'Something went wrong');
    }
  },
);
...
const todoSlice = createSlice({
  ...
  reducers: {
    ...
    optimisticStatusUpdate(state, action: PayloadAction<{ id: number; status: TodoStatus }>) {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchTags.fulfilled, (state, action) => { state.tags = action.payload; });
    ...
    builder
      ...
      .addCase(createTodo.rejected, (state, action) => { state.error = action.payload as string; });
    ...
    builder
      ...
      .addCase(updateTodo.rejected, (state, action) => { state.error = action.payload as string; });
    ...
    builder
      ...
      .addCase(deleteTodo.rejected, (state, action) => { state.error = action.payload as string; });
  },
});

export const { setFilter, clearError, optimisticStatusUpdate } = todoSlice.actions;
